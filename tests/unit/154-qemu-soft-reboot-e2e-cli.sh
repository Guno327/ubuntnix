#!/usr/bin/env bash
# tests/unit/154-qemu-soft-reboot-e2e-cli.sh —
# tests/e2e/040-qemu-soft-reboot-e2e.sh's CLI surface: --help, argument
# validation, and the documented skip-when-unavailable contract (GitHub
# issue #59; mirrors tests/unit/072-e2e-harness-cli.sh and
...
# takes naturally. tests/e2e/040-qemu-soft-reboot-e2e.sh itself is the
# real soft-reboot re-exec test, opt-in via UBX_E2E=1 (tests/README.md),
...
harness="$UBX_REPO_ROOT/tests/e2e/040-qemu-soft-reboot-e2e.sh"
[ -x "$harness" ] || {
