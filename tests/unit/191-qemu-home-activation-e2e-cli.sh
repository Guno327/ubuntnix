#!/usr/bin/env bash
# tests/unit/191-qemu-home-activation-e2e-cli.sh --
# tests/e2e/060-qemu-home-activation-e2e.sh's CLI surface: --help,
# argument validation, and the documented skip-when-unavailable contract
# (SPEC.md §9, §11 M5 exit criterion; GitHub issue #105 -- the live-QEMU
# home-activation e2e, a follow-up to #98's build-time-only home surface).
# Mirrors tests/unit/136-qemu-switch-e2e-cli.sh for
# tests/e2e/020-qemu-switch-e2e.sh almost exactly (this harness shares its
# CLI surface -- --image/--timeout/--max-boots/--no-kvm/--keep-log -- and
# its multi-boot design).
#
...
# exercises the harness's own argument handling and its graceful skip
# path, which is exactly the path this dev harness (no qemu installed)
# takes naturally. tests/e2e/060-qemu-home-activation-e2e.sh itself is the
# real home-activation boot test, opt-in via UBX_E2E=1 (tests/README.md),
# and is expected to skip (exit 77) right here on this host for the
# identical reason this test exercises.
set -u
...
harness="$UBX_REPO_ROOT/tests/e2e/060-qemu-home-activation-e2e.sh"
[ -x "$harness" ] || {
...
for flag in --help -h; do
  ...
  for word in --image --timeout --max-boots --no-kvm --keep-log; do
    case "$out" in
      *"$word"*) ;;
...
for opt in --image --timeout --max-boots --keep-log; do
  out="$("$harness" "$opt" 2>&1)"
  rc=$?
...
# -- a non-integer --timeout / --max-boots is rejected ---------------------
...
out="$("$harness" --max-boots not-a-number 2>&1)"
rc=$?
[ "$rc" -ne 0 ] || fail "--max-boots not-a-number should fail, got exit 0"
case "$out" in
  *"--max-boots"*) ;;
  *) fail "bad --max-boots error should mention '--max-boots', got: $out" ;;
esac

# -- --max-boots 0 is rejected (must follow at least one boot) -------------
out="$("$harness" --max-boots 0 2>&1)"
rc=$?
[ "$rc" -ne 0 ] || fail "--max-boots 0 should fail, got exit 0"
