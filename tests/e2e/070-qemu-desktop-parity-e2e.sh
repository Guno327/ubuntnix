#!/usr/bin/env bash
# tests/e2e/070-qemu-desktop-parity-e2e.sh — QEMU end-to-end desktop-parity
# boot test (SPEC.md §11 M6 exit criterion: "the desktop parity config boots
# in QEMU with a package set matching the upstream Desktop seed (minus
# enumerated exceptions)"; GitHub issue #108).
#
# Boots `.#desktop-parity-image`'s raw disk image (nix/profiles.nix's
# `desktopParityDiskImage`, built from examples/desktop.nix compiled through
# every landed M6 base module) in qemu-system-x86_64, headless, serial
# console captured to a log file, under a hard timeout, and asserts the log
# contains the distinctive UBX-DESKTOP-PARITY-PASS marker that image's own
# baked-in `ubx-desktop-parity-assert.service` emits once it has confirmed,
...
# CI (a new "desktop-parity" job, .github/workflows/ci.yml) has both.
set -u

prog_name="070-qemu-desktop-parity-e2e.sh"

usage() {
  cat <<USAGE
usage: 070-qemu-desktop-parity-e2e.sh [options]

Boots a ubuntnix desktop-parity disk image in QEMU and asserts it reaches
multi-user.target with the desktop-seed package set present (minus
enumerated exceptions) and cloud-init present-but-inert (SPEC.md §11 M6
...
options:
  --image PATH     raw disk image to boot, OR a directory containing
                    ...
                    \$UBX_DESKTOP_PARITY_IMAGE if set, else built on the fly
                    via \`nix build .#desktop-parity-image\` if \`nix\` is on
                    PATH.
...
main() {
  ...
  if [ -z "$image" ]; then
    image="${UBX_DESKTOP_PARITY_IMAGE:-}"
  fi
  ...
  if [ -z "$image" ]; then
    if command -v nix > /dev/null 2>&1; then
      built_dir="$(mktemp -d)"
      echo "$prog_name: no --image/\$UBX_DESKTOP_PARITY_IMAGE given; building .#desktop-parity-image via nix..." >&2
      if ! nix --extra-experimental-features 'nix-command flakes' build .#desktop-parity-image -o "$built_dir/result" -L; then
        rm -rf "$built_dir"
        die "nix build .#desktop-parity-image failed -- see output above"
      fi
      image="$built_dir/result"
    else
      skip "no --image/\$UBX_DESKTOP_PARITY_IMAGE given, and no 'nix' on PATH to build .#desktop-parity-image -- this dev harness cannot exercise the e2e boot (see tests/README.md's e2e contract)"
    fi
  fi
  ...
  if grep -q 'UBX-DESKTOP-PARITY-FAIL' "$log"; then
    echo "$prog_name: FAIL: guest reported UBX-DESKTOP-PARITY-FAIL -- last 60 lines of the serial log:" >&2
    tail -n 60 "$log" >&2
    rm -f "$log"
    exit 1
  fi

  if grep -q 'UBX-DESKTOP-PARITY-PASS' "$log"; then
    echo "$prog_name: PASS: found UBX-DESKTOP-PARITY-PASS in the serial console log" >&2
    rm -f "$log"
    exit 0
  fi

  echo "$prog_name: FAIL: UBX-DESKTOP-PARITY-PASS not found in the serial console log (qemu exit code $rc) -- last 60 lines:" >&2
  tail -n 60 "$log" >&2
  rm -f "$log"
  exit 1
}
