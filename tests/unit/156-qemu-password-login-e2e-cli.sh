#!/usr/bin/env bash
# tests/unit/156-qemu-password-login-e2e-cli.sh —
# tests/e2e/025-qemu-password-login-e2e.sh's CLI surface: --help,
# argument validation, and the documented skip-when-unavailable contract
# (GitHub issue #90, milestone M4's password-login e2e harness; mirrors
# tests/unit/136-qemu-switch-e2e-cli.sh for tests/e2e/020-qemu-switch-
# e2e.sh exactly, one level up the milestone).
#
...
# takes naturally. tests/e2e/025-qemu-password-login-e2e.sh itself is the
# real password-login boot test, opt-in via UBX_E2E=1 (tests/README.md),
...
harness="$UBX_REPO_ROOT/tests/e2e/025-qemu-password-login-e2e.sh"
[ -x "$harness" ] || {
...
for flag in --help -h; do
  ...
  case "$out" in
    *"UBX-M4-PW-PASS"*) ;;
    *) fail "'$flag' output should mention the UBX-M4-PW-PASS marker, got: $out" ;;
  esac
done
