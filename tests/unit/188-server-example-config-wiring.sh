#!/usr/bin/env bash
# tests/unit/188-server-example-config-wiring.sh — the server parity
# example config (examples/server.nix) actually wires the landed base
...
# it confirms examples/server.nix exists, declares every landed base
# module's own surface field (networking, fileSystems, swapDevices, i18n,
# console, time, users) plus profiles.server.enable, and that
...
# examples/server.nix must actually wire the landed base modules
# (networking, fileSystems+swap, i18n/console/time, users) plus
# profiles.server.enable -- SPEC.md §10's own worked-example shape, and
# this issue's own acceptance criteria ("server parity example config
# exercising all base modules").
...
grep -qE 'profiles\.server\.enable[[:space:]]*=[[:space:]]*true' "$example_config" ||
  fail "$example_config does not set profiles.server.enable = true"
...
# module's own render/validate function -- proving examples/server.nix's
...
for owner in 'flake.lib.networking' 'flake.lib.fileSystems' 'flake.lib.localization' 'flake.lib.users'; do
  grep -qF "config.$owner" "$profiles_nix" ||
    fail "$profiles_nix does not run examples/server.nix's fields through config.$owner"
done

# examples/server.nix must be pure declaration data -- the same purity
