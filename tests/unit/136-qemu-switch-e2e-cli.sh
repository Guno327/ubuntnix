#!/usr/bin/env bash
# tests/unit/136-qemu-switch-e2e-cli.sh — tests/e2e/020-qemu-switch-e2e.sh's
# CLI surface: --help, argument validation, and the documented skip-when-
# unavailable contract (GitHub issue #32, milestone M2's switch-loop e2e
# harness; mirrors tests/unit/072-e2e-harness-cli.sh for
# tests/e2e/010-qemu-boot-e2e.sh exactly, one level up the milestone).
#
...
# takes naturally. tests/e2e/020-qemu-switch-e2e.sh itself is the real
# switch-loop boot test, opt-in via UBX_E2E=1 (tests/README.md), and is
# expected to skip (exit 77) right here on this host for the identical
# reason this test exercises.
set -u
...
harness="$UBX_REPO_ROOT/tests/e2e/020-qemu-switch-e2e.sh"
[ -x "$harness" ] || {
