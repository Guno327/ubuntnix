#!/usr/bin/env bash
# tests/unit/151-qemu-snap-e2e-cli.sh — tests/e2e/030-qemu-snap-e2e.sh's CLI
# surface: --help, argument validation, and the documented skip-when-
# unavailable contract (GitHub issue #64, milestone M3's snap-convergence
# e2e harness; mirrors tests/unit/072-e2e-harness-cli.sh /
# tests/unit/136-qemu-switch-e2e-cli.sh exactly, one level up the
# milestone).
#
...
# naturally. tests/e2e/030-qemu-snap-e2e.sh itself is the real snap-
# convergence boot test, opt-in via UBX_E2E=1 (tests/README.md), and is
...
harness="$UBX_REPO_ROOT/tests/e2e/030-qemu-snap-e2e.sh"
[ -x "$harness" ] || {
...
# harness itself checks, so this assertion is meaningful regardless of what
# host ends up running this suite): the harness must SKIP (exit 77), not
# fail or hang, with a message identifying qemu as the missing piece.
